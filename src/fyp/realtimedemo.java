...
public class realtimedemo extends JFrame
{
    //
    ...
    realtimedemo()
    {
        DBConfig db=new DBConfig();
            Object data[]=db.getSuccessFailureCount();
            dataA=Double.parseDouble(data[0].toString());
            dataB=Double.parseDouble(data[1].toString());
            db=null;
    }
...
*/
        ...
        //retrieve the data
        retrievePB = new JButton("Retrieve", new ImageIcon(getClass().getResource("arrow.gif")));
        retrievePB.setHorizontalAlignment(SwingConstants.LEFT);
        retrievePB.setMargin(new Insets(5, 5, 5, 5));
        retrievePB.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt)    {
                retrievePB_Clicked();
            }});
        leftPanel.add(retrievePB).setBounds(1, 120, 138, 24);
        ...
        samplePeriod = new JComboBox(new Object[] { "250", "500", "750", "1000", "1250", "1500",
            "1750", "2000" });
        samplePeriod.setSelectedItem("3000");
        ...
        leftPanel.add(new JLabel("")).setBounds(5, 180, 130, 20);

        // Alpha Label
        JLabel alphaLabel = new JLabel("Success");
        ...
        // Beta label
        JLabel betaLabel = new JLabel("Failure");
...
*/
    ...
    private void dataRateTimer_Tick()
    {
        ...
        do
        {
           ...
           // dataA = 20 + Math.cos(p * 2.2) * 10 + 1 / (Math.cos(p) * Math.cos(p) + 0.01);
           // dataB = 150 + 100 * Math.sin(p / 27.7) * Math.sin(p / 10.1);
    ...
    private void retrievePB_Clicked()
    {
        DBConfig db=new DBConfig();
        Object data[]=db.getSuccessFailureCount();
        dataA=Double.parseDouble(data[0].toString());
    ...
    private void drawChart(ChartViewer viewer)
    {
        ...
        c.addTitle("Admin Review", "Times New Roman Bold Italic", 15
        ...
        c.yAxis().setTitle("No of Users", "Arial Bold", 10);
        ...
        if (lastTime != null)
        {
            ...
            // legend box.
            layer.addDataSet(dataSeriesA, 0x00cc00, "Success: <*bgColor=CCFFCC*>" +
                c.formatValue(dataSeriesA[dataSeriesA.length - 1], " {value|2} "));
            layer.addDataSet(dataSeriesB, 0xff0000, "Failure: <*bgColor=FFCCCC*>" +
